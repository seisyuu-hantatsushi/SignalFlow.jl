...
if [[ $# -lt 3 ]]; then
  ...
  echo "Env: FORCE_ON_SLOPE_EPS=5e-6 FORCE_ON_CPE_EPS_DEG=0.05"
...
force_on_slope_eps="${FORCE_ON_SLOPE_EPS:-5e-6}"
force_on_cpe_eps_deg="${FORCE_ON_CPE_EPS_DEG:-0.05}"
...
run_one() {
  local mode="$1"   # off|on
  local slope_force_eps="$2"
  local cpe_force_eps_deg="$3"
  local rep="$4"

  local snr_tag="${snr_db//./p}"
  local alpha_tag="${alpha//./p}"
  local log="logs/eval_m3_forceab_${profile}_${mode}_awgn${snr_tag}_alpha${alpha_tag}_r${rep}_th${unlock_th//./p}_uc${unlock_confirm}_$(echo "$carrier" | tr -d '.').log"
  log="${log%.log}_${ts}.log"
  run_logs+=("$log")

  echo "[run ${mode} r=${rep}/${repeats}] $log"
  cmd=(
...
for r in $(seq 1 "$repeats"); do
  run_one off 0 0 "$r"
  run_one on "$force_on_slope_eps" "$force_on_cpe_eps_deg" "$r"
done
